...
def GetArgs(d):
    ...
    d["problem_type"] = GetNumber("Problem? ", default=1, numtype=int, 
                                  low=1, high=3)
    d["h"] = flt(GetNumber("Height? ", default=1, low=0, low_open=True))
    if d["problem_type"] == 1:
        d["small_dia"] = flt(GetNumber("Small diameter? ", default=1, low=0, 
                                       low_open=True))
        d["large_dia"] = flt(GetNumber("Large diameter? ",
                                       default=d["small_dia"], low=0, 
                                       low_open=True))
    elif d["problem_type"] == 2:
        d["small_dia"] = flt(GetNumber("Small diameter? ", default=1, low=0, 
                                       low_open=True))
        d["theta_deg"] = flt(GetNumber("Included angle in degrees? ", default=10,
                                       low=0, low_open=True))
    else:
        d["large_dia"] = flt(GetNumber("Large diameter? ",
                                       default=d["small_dia"], low=0, 
