...
if 1:  # Core functionality
    ...
    def GetLoadAndDeflection():
        if g.dbg:
            ...
            s = shop_util.GetChoice(
                "Service type l=light, a=average, s=severe?", "a", ("l", "a", "s")
            )
            tau = GetAllowableWorkingStressInPsi(d, s)
            D = shop_util.GetDouble(
                "Mean spring diameter in inches?", "0.3", 0.005, 1e6
            )
            N = shop_util.GetInt("Number of active coils?", 20, 1, int(1e9))
            H = shop_util.GetDouble(
                "Free length of spring in inches?", "1.5", 2 * d, int(1e9)
            )
        h = N * d + d
        L = (H - d) / N
    ...
    def GetWireDiameter():
        ...
        OD = shop_util.GetDouble(
            "Spring outside diameter in inches?", "0.5", 0.005, 1e6
        )
        delta = shop_util.GetDouble("Deflection in inches?", "1", 0.001, 2000)
        H = shop_util.GetDouble("Free length of spring in inches?", "2", 0.01, int(1e9))
        ...
        while (abs(d - newd) / newd) > 0.0005:
            ...
            s = math.floor(
                43094 - (11037 * math.log(d))
            )  # Allowable stress, average service
        ...
        if d < dmin:
            print("** To meet required deflection, the spring would be overstressed **")
