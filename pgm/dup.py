        ...
        if not d["-L"]:
            ...
            for inode in links:
                if len(links[inode]) > 1:
                    ...
                    if d["-l"]:
                        print(f"{t.yell}Hard-linked [inode] or soft-linked <inode> files "
                            "(-l option used) (", file=stream, end="")
                    else:
                        print(f"{t.yell}Hard-linked files [inode number] (", file=stream, end="")
                    PrintSize(size, stream, d)
                    t.print("):")
                    for filename, dirnumber, islink in links[inode]:
                        li = (dirnumber, inode, filename)
                        if islink:
                            print("  %d:<%d>:  %s" % li, file=stream)
                        else:
                            print("  %d:[%d]:  %s" % li, file=stream)
                    print(file=stream)
        # Print out the true duplicates
        size = os.stat(item[0][0]).st_size
        print(f"{t.cynl}Duplicate files ", file=stream, end="")
