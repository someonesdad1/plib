...
* Convert to use flt, add -d for sig figures
* Helix angle
...
def ProcessCommandLine():
    ...
    try:
        optlist, args = getopt.getopt(sys.argv[1:], "Aa:bc:d:hlL:moptTuw:")
...
if old:
    def PrintBestWire(pitch):
        ...
        print(fmt % ("    Min wire size", min, min*mm_per_in))
        print(fmt % ("    Max wire size (%d%%)" % int(100*d['-a']), max,
              max*mm_per_in))
        print(fmt % ("    Best wire size", best_wire_size,
              best_wire_size*mm_per_in))
else:
    def PrintBestWire(pitch):
        best_wire_size = pitch/(2*cos(radians(30)))
        Min, Max = WireSizeLimits(pitch)
...
if old:
    ...
    def PrintResults(diameter, tpi):
        ...
        print(" " * spc, "   inches        mm")
        print(" " * spc, "   ------        ---")
        print(fmt % ("Nominal diameter", diameter, diameter*mm_per_in))
        print(fmt1 % ("Threads per inch or mm", tpi, tpi/mm_per_in))
        # Length of engagement
        loe = diameter*d["-L"]
        print(fmt % ("Length of engagement", loe, loe*mm_per_in))
        # Sellers' recommended tpi given the diameter
        p = 0.24*sqrt(diameter + 5/8) - 0.175
        if diameter < 1/4:
            p *= 0.23/0.24
        print(fmt2 % ("Sellers' recommended tpi", 1/p))
        print(vf % ("Thread %slass" % "c"), "  %2d" % thread_class)
        print(fmt % ("Pitch = p", 1/tpi, mm_per_in/tpi))
        print(fmt % ("H = sqrt(3)/2*p", H, mm_per_in*H))
        print(fmt % ("Vee thread double depth = 2*H", 2*H, 2*mm_per_in*H))
        print(fmt % ("Allowance", A.Allowance(), A.Allowance()*mm_per_in))
        # Thread tensile strength area (ASME 1989, Appendix B)
        a = diameter - 0.9743*p
        AS = pi/4*a*a
        print(fmt6 % ("Tensile area, in^2 or mm^2", AS, AS*mm_per_in*mm_per_in))
        # Thread shear area
        ...
        print(fmt % ("        Max", A.Dmax(), A.Dmax()*mm_per_in))
        print(fmt % ("        Min", A.Dmin(), A.Dmin()*mm_per_in))
        print(fmt % ("        Tolerance", A.Dmax()-A.Dmin(),
            (A.Dmax()-A.Dmin())*mm_per_in))
        print("    Pitch diameter")
        print(fmt % ("        Max", A.Emax(), A.Emax()*mm_per_in))
        print(fmt % ("        Min", A.Emin(), A.Emin()*mm_per_in))
        print(fmt % ("        Tolerance", A.Emax()-A.Emin(),
            (A.Emax()-A.Emin())*mm_per_in))
        print("    Minor diameter, vee thread")
        md = 0.5*(A.Dmax() - 2*H + A.Dmin() - 2*H)
        print(fmtv % ("        Nominal", md, md*mm_per_in))
        print()
        #
        ...
        print(fmt % ("        Meas. over best wires, max", mow_max,
            mow_max*mm_per_in))
        print(fmt % ("        Meas. over best wires, min", mow_min,
            mow_min*mm_per_in))
        if d["-w"]:
            W = d["-w"]  # Wire diameter
            s = "User-specified wire diameter"
        ...
        if W:
            ...
            print(fmt % ("        Meas. over wires, max", mow_max,
                mow_max*mm_per_in))
            print(fmt % ("        Meas. over wires, min", mow_min,
                mow_min*mm_per_in))
            print()
        print("Tap drills")
        fmtf = fmt + "    %s"
        for percent in range(50, 101, 5):
            ...
            print(fmtf % ("    %3d%% thread" % percent, D, D*mm_per_in, F))
        print()
        ThreadDepths(H)
else:
    # New version using f-strings and flt
    def mm(x):
        return flt(25.4*x)
    ...
    def ThreadDepths(H):
        ...
        if 1:
            ...
            print("Sharp V thread with National Form flat on OD, "
                "compound at 29°")
            print(fmt3 % ("    Double depth = 7*A/8", 2*B, 2*B*mm_per_in))
            print(fmt3 % ("    Compound feed = A = sqrt(3)*p",
                2*B/C, 2*B*mm_per_in/C))
            print()
            print("Unified thread, compound at 29° (cos(29°) = 7/8)")
            print(fmt3 % ("    Flat on form tool", pitch/8, pitch/8*mm_per_in))
            print(fmt3 % ("    Double depth", 2*A, 2*A*mm_per_in))
            print(fmt3 % ("    Compound feed, DD/cos(29°)", 2*A/C, 2*A*mm_per_in/C))
        else:
