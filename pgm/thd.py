...
if 1:  # Core functionality
    ...
    def SearchTPI(args):
        ...
        for name, OD, TPI in weird_threads:
            if abs(tpi - TPI) / tpi < tolerance:
                ...
                long_name = weird_thread_names[name.split()[-1]]
                print(
                    '    %-11s  OD = %6.4f", tpi = %.1f %s' % (name, OD, TPI, long_name)
                )
        if not found:
            print("  No matches")
        exit(0)
    ...
    def ThreadDepths(H):
        ...
        if 1:
            ...
            with A:
                A.n = max(d["-d"] - 1, 1)
                print("Sharp V thread with National Form flat on OD, compound at 29°")
    ...
    def PrintThread(thd):
        ...
        for klass in thd[3]:
            ...
            Emin = T.Emin()
            print(
                "%-14s %dA " % (name, Class) + 4 * fmt % (Dmax, Dmin, Emax, Emin),
                end="",
            )
            print("  ", end="")
            # Internal thread
    ...
    def PrintResults(diameter, tpi):
        ...
        A = asme.UnifiedThread(
            diameter, tpi, thread_class, length_of_engagement=d["-L"]
        )
        p = 1 / tpi
        H = p * sqrt(3) / 2
        if 0:
