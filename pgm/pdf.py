...
if 1:  # Core functionality
    ...
    def ToText(*files):
        for file in files:
            pdf = P(file)
            cmd = ["pdftotext", "-enc UTF-8", "-layout", str(pdf)]
            s = " ".join(cmd)
    ...
    def ToHTML(*files):
        ...
        pdf = P(files[0])
        cmd = ["pdftohtml", str(pdf), str(files[1])]
        s = " ".join(cmd)
        r = subprocess.run(s, shell=True, capture_output=True)
        if r.returncode:
            t.print(f"{t.err}{s!r} failed:", file=sys.stderr)
    ...
    def PNG(*files):
        ...
        pdf = P(files[0])
        cmd = ["pdftopng", str(pdf), str(files[1])]
        s = " ".join(cmd)
        r = subprocess.run(s, shell=True, capture_output=True)
        if r.returncode:
            t.print(f"{t.err}{s!r} failed:", file=sys.stderr)
    ...
    def Images(*files):
        ...
        pdf = P(files[0])
        cmd = ["pdfimages", str(pdf), str(files[1])]
        s = " ".join(cmd)
        r = subprocess.run(s, shell=True, capture_output=True)
        if r.returncode:
            t.print(f"{t.err}{s!r} failed:", file=sys.stderr)
