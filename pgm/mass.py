...
Overall design change
    - Make this a python module that you simply write a code file to use; this allows
      the use of other modules for more flexibility.
    - Global variables:
        - material = "concrete"
        - units = "cm"
        - angles = "degrees"
        ...
        - mass_unit = "kg"
        - volume_unit = "ml"
    - A list is a container for a group of components
        
        clamp = []
        clamp.append(cyl(d=3/4, L=15, material"steel", name="shaft 1"))
        clamp.append(cyl(d=3/4, L=15, material"steel", name="shaft 2"))

    - Then you call Report(*containers) to get a report on the whole

    - For a list of bar stock materials, a shorthand list can be made from a string:

        s = """
            rnd d=3/4, L=15, material"steel", name="shaft 1"
            hex d=3/4, L=15, material"steel", name="shaft 2"
            hex r=18 mm, L=5i00 mm, material"steel", name="shaft 2"
            ...
        """
        container = BarStock(s)
    
        - Note how the units are allowed, this time using Num objects, as this also
          allows the use of uncertainty

    - Reasoning:  this makes the code much more simple, as there's no interpretation of
      commands and maintaining a dict of variables.  It also allows the full use of
      python's syntax as needed.


- Bugs
    - % of total column is wrong for mass.leadhammer file

- ToDo

    - MUST
        - Use Num for variables
        - Add .on and .off to datafile
        - Add a --debug option to show exactly what's going on
        - Get rid of sig
        - Change to termtables for report
    - WANT
        - For barstock calculations:  add the keywords rnd, hex, sq, oct, rect
        - Add 'sq', 'square' for square bar stock
            - Let 'bar' be a synonym of 'cyl'
        
    - Good example is to calculate mass of 1-2-3 block with all of its holes
        - 15 holes in big face, 5 in medium face, 3 in small
        - 5 big face holes are tapped 3/8-16 with 0.314 ∅ ID; others are 0.343(5) ∅ ID; each
          hole has a countersink 0.475 ∅, 0.344 ∅ at bottom for no tap, 0.315 ∅ at
          bottom for tapped, 0.1 deep
...
if 1:  # Classes
    ...
    class Primitive(object):
        ...
        ignore = None
        # Used where steel is the default material 
        steel_sp_gr = 7.85 # g/cc
        def __init__(self, linenum, line, opts, **kw):
            '''The dictionary kw will contain the numbers that were given in the
            datafile line.  Make sure we have a corresponding diameter if a radius was
            given.
...
if 1:   # Core functionality
    def PrintDatafileExample(d):
        print(dedent(f'''
        # For a list of the keywords, modifiers, and control keywords, use the -h
        # command line option.  Also consult the mass.pdf instructions.
        
        # This is an example datafile for the mass.py script.  It can be used as a
        # template for a particular assembly by modifying and adding to the included
        # items.
        
        # Material:  Use the -l option for a listing of the available materials.  Use
        # the -L 3 option to see all materials supported by the script.
        material concrete
        
        # Units:  define the default length units.  You can change units at any time by
        # using a new units statement.  Most common length units are supported; SI
        # prefixes can be used.
        units cm
        
        # Angles:  define the angle units.  This control keyword defines the factor that
        # converts angle measure to radians.  To have all angles in degrees, use the
        # following line.
        angles pi/180
        
        # Digits:  the number of significant figures in the printed report is controlled
        # by this number.
        digits 3
        
        # Mass unit:  this control keyword defines the mass unit to be used in the
        # report.
        munit kg
        ...
        #----------------------------------------------------------------------
        # Components:  each component of the composite assembly is defined by a
        # component line.  Each line requires certain keywords to specify the dimension
        # of the component.  Each keyword must be of the form 'kw = value', followed by
        # a comma.  Keywords can be:
        #
        ...
        # Here's an example that specifies a bar of steel with a cylindrical cross
        # section 3/4 length units in diameter and 15 length units long.  The component
        # has the name "shaft 3".
        cyl d=3/4, L=15, material="steel", name="shaft 3"
        
        # Note:  when a diameter is needed, either the radius r can be given or the
        # diameter d can be given.  To understand all the keywords, you'll need to look
        # at the diagrams in the mass.pdf file.
        ...
        # 2x8
        # and the following Torx wood screw sizes (name is length in 0.01 inches):
    ...
    def PrintMaterials(d):
        ...
        print("  Gases are at 20 °C and 1 atmosphere pressure")
