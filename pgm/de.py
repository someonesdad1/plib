...
if 1:  # Core functionality
    ...
    def WrenchSizes(d):
        ...
        for i, inches in enumerate(inch):
            ...
            s = (
                f"{sep} {x}{str(inches):^6s} {float(inches):6.3f} "
    ...
    def TapDrills(d):
        ...
        T = (
            "#0,5/32 #1,5/32 #2,3/16 #3,3/16 #4,1/4 #5,5/16 #6,5/16 #8,11/32 "
        ...
        T = (
            "#1,1/8 #2,1/8 #3,3/16 #4,3/16 #5,3/16 #6,1/4 #8,1/4 "
        ...
        T = (
            "#0,0.05 #1,1/16 #2,5/64 #3,5/64 #4,3/32 #5,3/32 #6,7/64 #8,9/64 "
        ...
        T = (
            "M2,4 M2.5,5 M3,5.5 M3.5,6 M4,7 M5,8 M6,10 M7,11 M8,13 M10,16 "
        ...
        for size, wrench in [i.split(",") for i in T.split()]:
            ...
            s.append([d, "{} hex head".format(size), GN["hexh"]])
        T = (
            "M2,1.5 M2.5,2  M3,2.5 M4,3  M5,4  M6,5  M8,6  M10,8  M12,10 "
        ...
        for size, wrench in [i.split(",") for i in T.split()]:
            ...
            s.append([d, "{} SHCS head".format(size), GN["shcs"]])
        T = (
            "M2,1.25 M2.5,1.5 M3,2 M4,2.5 M5,3 M6,4 M8,5 M10,6 M12,8 M16,10 "
        ...
        T = (
            "M2,0.9 M2.5,1.3 M3,1.5 M4,2 M5,2.5 M6,3 M8,4 M10,5 M12,6 "
            "M16,8 M20,10 M24,12"
        )
        for size, wrench in [i.split(",") for i in T.split()]:
            d = round(float(wrench) / 25.4, 4)
