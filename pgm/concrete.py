...
if 1:   # Shapes
    ...
    class Slab(FormShape):
        ...
        def GetDimensions(self):
            ...
            self.how_many = GetNumber("How many of them? ", numtype=int,
                                    low=1, default=1, allow_quit=True)
        def Characteristics(self):
            return (
    ...
    class HorizontalCylinder(FormShape):
        ...
        def GetVolume(self):
            ...
            a = r - h
            return L*(r*r*acos(a/r) - a*sqrt(2*r*h - h*h))*self.how_many
        def GetDimensions(self):
            self.diameter, self.diameter_orig = GetLength("Diameter of cylinder? ")
            self.length, self.length_orig = GetLength("Length of cylinder? ")
            self.percent, self.percent_orig = GetNumber(
                "Filled to percent of height? ", low=0, low_open=True,
    ...
    class Sphere(FormShape):
        ...
        def GetDimensions(self):
            ...
            self.percent, self.percent_orig = GetNumber(
                "Filled to percent of diameter? ", low=0, low_open=True,
                high=100, use_unit=False)
            self.how_many = GetNumber("How many of them? ", numtype=int,
                                    low=1, default=1, allow_quit=True)
        def Characteristics(self):
