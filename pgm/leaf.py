...
if 1:   # Core functionality
    def GetInternalImports(file_path, all_module_names):
        ...
        try:
            ...
            t.print(f"{t.err}Could not parse {file_path}: {e}")
    ...
    def AnalyzePlib():
        ...
        '''
        # Colors to use
        t.title = t("yel1", attr="ul")
        t.leaf = t.sky
        t.emph = t("red", attr="ul")
        t.most_used = t.orn
        # 
        t.print(f"{t.title}Analysis of /plib modules (in /plib & /plib/data)\n")
        if 1:   # 1. Get list of all python files
        ...
        if 1:   # 2. Build the dependency map
            dep_graph = {}
            for py_file in py_files:
                module_name = py_file.stem
                deps = GetInternalImports(py_file, all_module_names)
                ...
                deps.discard(module_name)
                dep_graph[module_name] = deps
        if 1:   # 3. Identify the "Foundations" (Leaf nodes with 0 internal dependencies)
            foundations = [m for m, d in dep_graph.items() if len(d) == 0]
            t.print(f"{t.emph}Leaf Nodes{t.n}  {t.sky}Files that depend on no other internal modules:")
            for i in Columnize(sorted(foundations), indent=" "*2):
                print(i)
        if 1:   # 4. Identify "High-Impact" modules (heavily imported by others)
            usage_counts = {m: 0 for m in all_module_names}
            for deps in dep_graph.values():
                for d in deps:
                    usage_counts[d] += 1
            t.print(f"{t.most_used}Most used modules:")
            sorted_impact = sorted(usage_counts.items(), key=lambda x: x[1], reverse=True)
            for m, count in sorted_impact[:20]:
                if count > 0:
