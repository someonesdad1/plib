...
if 1:   # Global variables
    ...
    # values per line.
    R_kohm = [flt(i) for i in (
        884.6,830.9,780.8,733.9,690.2,649.3,611,575.2,541.7,510.4,
        481,453.5,427.7,403.5,380.9,359.6,339.6,320.9,303.3,286.7,
        271.2,256.5,242.8,229.8,217.6,206.2,195.4,185.2,175.6,166.6,
        158,150,142.4,135.2,128.5,122.1,116,110.3,104.9,99.8,
        94.98,90.41,86.09,81.99,78.11,74.44,70.96,67.66,64.53,61.56,
        58.75,56.07,53.54,51.13,48.84,46.67,44.6,42.64,40.77,38.99,
        37.3,35.7,34.17,32.71,31.32,30,28.74,27.54,26.4,25.31,
        24.27,23.28,22.33,21.43,20.57,19.74,18.96,18.21,17.49,16.8,16.15,
        15.52,14.92,14.35,13.8,13.28,12.77,12.29,11.83,11.39,
        10.97,10.57,10.18,9.807,9.45,9.109,8.781,8.467,8.166,7.876,7.599,
        7.332,7.076,6.83,6.594,6.367,6.149,5.94,5.738,5.545,
        5.359,5.18,5.007,4.842,4.682,4.529,4.381,4.239,4.102,3.97,
        3.843,3.72,3.602,3.489,3.379,3.273,3.172,3.073,2.979,2.887,
        2.799,2.714,2.632,2.552,2.476,2.402,2.331,2.262,2.195,2.131,
        2.069,2.009,1.95,1.894,1.84,1.788,1.737,1.688,1.64,1.594,
        1.55,1.507,1.465,1.425,1.386,1.348,1.311,1.276,1.241,1.208,
        1.176,1.145,1.114,1.085,1.057,1.029,1.002,0.9763,0.9511,0.9267,0.903,
        0.88,0.8577,0.8361,0.815,0.7946,0.7748,0.7556,0.7369,0.7188,
        0.7012,0.6841,0.6675,0.6513,0.6356,0.6203,0.6055,0.5911,0.5771,0.5635,0.5502
    )]
    ...
    RS_kOhms = (
        329.2, 247.5, 188.4, 144.0, 111.3, 86.39, 67.74, 53.39, 42.45, 33.89,
        27.28, 22.05, 17.96, 14.68, 12.09, 10.00, 8.313, 6.941, 5.828, 4.912,
        4.161, 3.537, 3.021, 2.589, 2.229, 1.924, 1.669, 1.451, 1.266, 1.108,
        0.9735, 0.8575, 0.7579
    )
...
def RegressionData():
    ...
    for t, R in zip(range(-40, 151), R_kohm):
        ...
        r = t - Tpred + k
        resid.append(r)
    print(f"# Min/max residuals for regression fit in K:  {min(resid):.2f}, {max(resid):.2f}")
if __name__ == "__main__":
    ...
    for r in args:
        ...
        Tk = Tc + 273.15
        res = r + " kΩ"
        print(f"{res:8s}      T = {g.c}{Tc:.1f} °C{g.n} = "
