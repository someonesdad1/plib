...
if 1:  # Utility
    ...
    def ParseCommandLine(d):
        ...
        d["-s"] = (
            "bash"  # Default shell syntax (bash dash, ksh, sh), None means to infer
        )
        d["-v"] = False  # Debugging
        if len(sys.argv) < 2:
            Usage()
...
if 1:  # Classes
    ...
    class Output:
        ...
        def __init__(self, file, stdout, stderr):
            ...
            self.filelines = [
                (i + 1, line) for i, line in enumerate(GetLines(file, nonl=True))
            ]
            ...
            for line in stdout.decode().split("\n"):
                if line.strip():
                    ...
                    if d["-c"] and el.errnum == "SC2154":
                        ...
                        c = el.msg.split()[0]
                        ignore = set(
                            """redd red redl ornd orn ornl grnd grn grnl yeld yel yell
