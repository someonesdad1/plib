...
if 1:  # Utility
    def GetColors():
        t.input = t.wht if d["-c"] else ""
        t.frac = t.brnl
        t.out = t.seal if d["-c"] else ""
        t.mm = t.denl if d["-c"] else ""
        t.err = t.redl if d["-c"] else ""
        t.N = t.n if d["-c"] else ""
...
if 1:  # Core functionality
    ...
    def GetFormat(max_length, d):
        rnd = 2 if d["-m"] else 4
        return f"{t.input}{{0:^{0}s}}{t.N}  {t.out}{{1:.{1}f}}{t.N}".format(max_length, rnd)
    def PrintTable(d):
        metric = d["-m"]
        ...
        for size, name in sz:
            if "mm" in name:
                s1 = f"{t.mm}{name:^{max_length}s}{t.N}"
            else:
                if name == "1/1":
                    name = "1"
                if "/" in name:
                    s1 = f"{t.frac}{name:^{max_length}s}{t.N}"
                else:
                    s1 = f"{t.input}{name:^{max_length}s}{t.N}"
            if isinstance(size, Fraction):
                s2 = f"{t.out}{float(size):.{rnd}f}{t.N}"
            else:
                s2 = f"{t.out}{size:.{rnd}f}{t.N}"
            out.append(f"{s1}  {s2}")
        s = "Drill sizes in" + (" mm" if metric else " inches")
        print(f"{t.ornl}{s}{t.N}")
    ...
    def PrintSize(size, d):
        ...
        if metric:
            print(
                " " * indent,
                "Target size = ",
                "{0:.2f} mm".format(sz_inches * 25.4),
                sep="",
            )
        else:
            print(
                " " * indent, "Target size = ", "{0:.4f} inches".format(sz_inches), sep=""
            )
        sz = [(size, name) for name, size in GetDrillSizes(metric=d["-m"]).items()]
        sz.sort()
        s = []
