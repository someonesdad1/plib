...
if 1:  # Core functionality
    ...
    def TaperSizes():
        ...
        for id in tapers:
            ...
            t.print(
                f"{t.hdr}{i}Num{'D ':>{w}s} {'d ':>{w}s} {'L ':>{w}s} {'θ, °':>{w}s}  "
                f"{'tpi/2':>{w}s}{'tpf':>{w}s}"
            )
            ...
            for N in T.sizes:
                ...
                if impt:
                    print(f"{t.impt}", end="")
                print(
                    f"{i}{N:2d} {f(D):>{w}s} {f(d):>{w}s} {f(L):>{w}s} {θ:>{w}.4f}"
                    f"  {1000 * tpi / 2!s:>{w}s} {tpf!s:>{w}s}",
                    end="",
                )
    ...
    def Measured():
        ...
        if dbg:
            ...
            L, units = GetNumber(
                "  Length? ", low=0, low_open=True, allow_quit=True, use_unit=True
            )
            if units:
                L *= u(units) / u("inch")
            print()
