...
def Renard(s):
    ...
    d = {
        ...
        "R20": [1, 1.25, 1.6, 2, 2.5, 3.15, 4, 5, 6.3, 8,
                1.12, 1.4, 1.8, 2.24, 2.8, 3.55, 4.5, 5.6, 7.1, 9],
        "R40": [1, 1.25, 1.6, 2, 2.5, 3.15, 4, 5, 6.3, 8,
                1.06, 1.32, 1.7, 2.12, 2.65, 3.35, 4.25, 5.3, 6.7, 8.5,
                1.12, 1.4, 1.8, 2.24, 2.8, 3.55, 4.5, 5.6, 7.1, 9,
                1.18, 1.5, 1.9, 2.36, 3, 3.75, 4.75, 6, 7.5, 9.5],
        "R80": [1, 1.25, 1.6, 2, 2.5, 3.15, 4, 5, 6.3, 8,
                1.03, 1.28, 1.65, 2.06, 2.58, 3.25, 4.12, 5.15, 6.5, 8.25,
                1.06, 1.32, 1.7, 2.12, 2.65, 3.35, 4.25, 5.3, 6.7, 8.5,
                1.09, 1.36, 1.75, 2.18, 2.72, 3.45, 4.37, 5.45, 6.9, 8.75,
                1.12, 1.4, 1.8, 2.24, 2.8, 3.55, 4.5, 5.6, 7.1, 9,
...
def GetData(opts):
    ...
    sizes = Check((
        ...
        1910, 1890, 1850, 1820, 1800, 1770, 1730, 1695, 1660, 1610, 1590,
        1570, 1540, 1520, 1495, 1470, 1440, 1405, 1360, 1285, 1200, 1160,
        1130, 1110, 1100, 1065, 1040, 1015, 995, 980, 960, 935, 890, 860,
        820, 810, 785, 760, 730, 700, 670, 635, 595, 550, 520, 465, 430,
        420, 410, 400, 390, 380, 370, 360, 350, 330, 320, 310, 293, 280,
        260, 250, 240, 225, 210, 200, 180, 160, 145, 135
    ))
    ...
    # Sharpe Gauge.
    sizes = Check((
        32490, 28930, 25760, 22940, 20430, 18190, 16200, 14430, 12850,
        11440, 10190, 9070, 8080, 7200, 6410, 5710, 5080, 4530, 4030, 3590,
        3200, 2850, 2530, 2260, 2010, 1790, 1590, 1420, 1260, 1130, 1000,
        893, 795, 708, 630, 561, 500, 445, 396, 353, 314, 280, 249, 222,
        198, 176, 157, 140, 124, 111, 99
    ))
    for i, sz in enumerate(sizes):
    ...
    sizes = Check((
        3065, 2830, 2625, 2437, 2253, 2070, 1920, 1770, 1620, 1483, 1350,
        1205, 1055, 915, 800, 720, 625, 540, 475, 410, 348, 317, 286, 258,
        230, 204, 181, 173, 162, 150, 140, 132, 128, 118, 104, 95, 90, 85,
        80, 75, 70, 66, 62, 60, 58, 55, 52, 50, 48, 46, 44
    ))
    ...
    # Sheet steel 3-27
    sizes = Check((
        0, 0, 0, 2391, 2242, 2092, 1943, 1793, 1644, 1495, 1345, 1196,
        1046, 897, 747, 673, 598, 538, 478, 418, 359, 329, 299, 269, 239,
        209, 179, 164
    ))
    for i, sz in enumerate(sizes):
        if i < 3:
    ...
    sizes = Check((
        3400, 3000, 2840, 2590, 2380, 2200, 2030, 1800, 1650, 1480, 1340,
        1200, 1090, 950, 830, 720, 650, 580, 490, 420, 350, 320, 280, 250,
        220, 200, 180, 160, 140, 130, 120, 100, 90, 80, 70, 50, 40
    ))
    ...
    sizes = Check((
        227, 219, 212, 207, 204, 201, 199, 197, 194, 191, 188, 185, 182,
        180, 178, 175, 172, 168, 164, 161, 157, 155, 153, 151, 148, 146,
        143, 139, 134, 127, 120, 115, 112, 110, 108, 106, 103, 101, 99, 97,
        95, 92, 88, 85, 81, 79, 77, 75, 72, 69, 66, 63, 58, 55, 50, 45, 42,
        41, 40, 39, 38, 37, 36, 35, 33, 32, 31, 30, 29, 27, 26, 24, 23, 22,
        20, 18, 16, 15, 14, 13
    ))
    ...
    sizes = Check((
        ...
        39, 41, 43, 45, 47, 49, 51, 55, 59, 63, 67, 71, 75, 80, 85, 90, 95,
        100, 106, 112, 118, 124, 130, 138, 146, 154, 162, 170, 180
    ), neg_slope=False)
    ...
    sizes = Check((
        0, 0, 0, 2391, 2242, 2092, 1943, 1793, 1644, 1495, 1345, 1196,
        1046, 897, 747, 673, 598, 538, 478, 418, 359, 329, 299, 269,
        239, 209, 179, 164, 149, 135, 120, 105, 97, 90, 82, 75, 67, 64,
        60
    ))
    ...
    sizes = Check((
        ...
        710, 635, 575, 516, 456, 396, 366, 336, 306, 276, 247, 217, 202,
        187, 172, 157, 142, 134
    ))
    ...
    sizes = Check((
        ...
        1160, 1040, 920, 800, 720, 640, 560, 480, 400, 360, 320, 280, 240,
        220, 200, 180, 164, 148, 136, 124, 116, 108, 100, 92, 84, 76, 68,
        60, 52, 48, 44, 40, 36, 32, 28, 24, 20, 16, 12, 10
    ))
    ...
    sizes = Check((
        .18, .165, .148, .134, .12, .109, .095, .083, .072, .065, .058,
        .050, .042, .03575, .03225, .02825, .02525, .02225, .02025,
        .01825, .01625, .01425, .01325, .01225, .01025, .00925, .00825,
        .00725
    ))
    ...
    for sz, mm in {
            "6/0": 0.7, "5/0": 1, "4/0": 1.2, "3/0": 1.5, "2/0": 1.75,
            "0": 2.0, "1": 2.25, "2": 2.75, "3": 3.25, "4": 3.5, "5":
            3.75, "6": 4.0, "7": 4.5, "8": 5.0, "9": 5.5, "10": 6.0, 
            "10 1/2": 6.5, "11": 8.0, "13": 9.0, "15": 10.0, "17": 12.0,
    ...
    # https://en.wikipedia.org/wiki/Knitting_needle#Needle_sizes_and_conversions
    for sz, mm in {
            "14": 2.0, "13": 2.25, "12": 2.75, "3": 3.0, "10": 3.25,
            "9": 3.75, "8": 4.0, "7": 4.5, "6": 5.0, "5": 5.5, "4": 6.0,
            "3": 6.5, "2": 7.0, "1": 7.5, "0": 8.0, "2/0": 9.0, "3/0":
            10.0}.items():
        dia_m = RoundOff(mm*u("mm"))
    ...
    # https://en.wikipedia.org/wiki/Knitting_needle#Needle_sizes_and_conversions
    for sz, mm in {
            "0": 2.1, "1": 2.4, "2": 2.7, "3": 3.0, "4": 3.3, "5": 3.6,
            "6": 3.9, "7": 4.2, "8": 4.5, "9": 4.8, "10": 5.1, "11":
    ...
    for sz, OD in (
        ("#0", 0.096), ("#1", 0.118), ("#2", 0.140), ("#3", 0.161),
        ("#4", 0.183), ("#5", 0.205), ("#6", 0.226), ("#8", 0.270),
        ("#10", 5/16), ("1/4", 3/8), ("5/16", 15/32), ("3/8", 9/16),
        ("7/16", 21/32), ("1/2", 3/4), ("5/8", 15/16), ("3/4", 9/8),
        ("7/8", 21/16), ("1", 3/2)
    ):
        dia_m = RoundOff(OD*u("in"))
    ...
    for sz, OD in (
        ("#0", 0.06), ("#1", 0.073), ("#2", 0.086), ("#3", 0.099),
        ("#4", 0.112), ("#5", 0.125), ("#6", 0.138), ("#8", 0.164),
        ("#10", 0.190), ("1/4", 1/4), ("5/16", 5/16), ("3/8", 3/8),
        ("7/16", 7/16), ("1/2", 1/2), ("5/8", 5/8), ("3/4", 3/4),
        ("7/8", 7/8), ("1", 1)
    ):
        dia_m = RoundOff(OD*u("in"))
    ...
    for sz, OD in (
        ("#0", 0.05), ("#1", 1/16), ("#2", 5/64), ("#3", 5/64),
        ("#4", 3/32), ("#5", 3/32), ("#6", 7/64), ("#8", 9/64),
        ("#10", 5/32), ("1/4", 3/16), ("5/16", 1/4), ("3/8", 5/16),
        ("7/16", 3/8), ("1/2", 3/8), ("5/8", 1/2), ("3/4", 5/8),
        ("7/8", 3/4), ("1", 3/4)
    ):
        dia_m = RoundOff(OD*u("in"))
    ...
    for sz, OD in (
        ("#0", 5/32), ("#1", 5/32), ("#2", 3/16), ("#3", 3/16), ("#4", 1/4),
        ("#5", 5/16), ("#6", 5/16), ("#8", 11/32), ("#10", 3/8), ("1/4", 7/16),
        ("5/16", 1/2), ("3/8", 9/16), ("7/16", 11/16), ("1/2", 3/4),
        ("5/8", 15/16), ("3/4", 9/8), ("7/8", 21/16), ("1", 3/2)
    ):
        dia_m = RoundOff(OD*u("in"))
...
def Sockets(opts):
    ...
    if 0:
        ...
        hf, ac, pr = "Harbor Freight", "Action", "Proto"
        du = "Duro"
    else:
        # Abbreviations
