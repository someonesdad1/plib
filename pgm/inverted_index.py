...
def termsearch(terms):  # Searches simple inverted index
    ...
    """
    return reduce(
        set.intersection, (invindex[term] for term in terms), set(texts.keys())
    )
...
def phrasesearch(phrase):
    ...
    for txt in termsearch(wordsinphrase):
        ...
        for firstindx in (indx for t, indx in finvindex[firstword] if t == txt):
            ...
            if all(
                (txt, firstindx + 1 + otherindx) in finvindex[otherword]
...
if __name__ == "__main__":
    ...
    try:
        ...
        pp(sorted(words))
        invindex = {
            word: set(txt for txt, wrds in texts.items() if word in wrds)
            for word in words
        }
        print("\nInverted Index")
        ...
        ans = Counter(ans)
        print(
            "  The phrase is found most commonly in text: "
