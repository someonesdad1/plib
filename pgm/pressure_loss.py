...
if 1:   # Global variables
    ...
    velocities = [round(float(x), 3) for x in '''
...
def ParseCommandLine(d):
    ...
    for o, a in opts:
        ...
        if o in ("-d",):
            ...
            try:
                ...
                m = "'{}' is not a valid temperature in deg C"
                Error(m.format(a))
            if not (0 <= d["-t"] <= 100):
    ...
    if d["-d"] is None:
        d["-d"] = WaterDensity(d["-t"])
    if d["-v"] is None:
        d["-v"] = WaterDynamicViscosity(d["-t"])
...
def GetDiameter(dia, d):
    ...
    if dia in US_pipe_sizes:
        return ID_steel_sch_40[dia]/1000*0.0254
    else:
        # The default unit is inches; see if we can convert to a float.
        try:
            d = float(dia)
            return d*u("inches")
        except ValueError:
            return GetQuantity(dia, dim="m")
...
def PrintTable(diameter_m, d):
    ...
    g = 9.80665         # Standard acceleration of gravity in m/s2
    ...
    for v in velocities:
        D, Q = diameter_m, A*v
        Re = rho*v*diameter_m/mu        # Reynolds number
        v_mps = sig(v)
        Q_Lps = sig(A*v/u("L/s"))
        Q_gpm = sig(A*v/u("gpm"))
        m_kgps = sig(A*v*rho)
