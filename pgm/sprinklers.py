...
if 1:  # Core functionality
    ...
    def FormatTime(t):
        ...
        h %= 12
        m %= 60
        ap = "pm" if FormatDatetime > 11 else "am"
        if not h:
            h = 12
        return f"{h:2d}:{m:02d} {ap}"
    def FormatDatetime(tm):
    ...
    def GetStartTime(start_time):
        ...
        def Keep1(x, keep):
            'Generate a string from call to dpstr:Keep()'
            s = Keep(st, keep)
            return ''.join(list(s))
        try:
            if "." in st:
                # Decimal form
                if "p" in st:
                    h = float(Keep1(st, keep)) + 12
                    m = int(60*(h - int(h)))
                    h = int(h)
                else:
                    h = float(Keep1(st, keep))
                    m = int(60*(h - int(h)))
                    h = int(h)
            elif ":" in st:
                if "a" in st:
        ...
        def Print():
            ...
            for ckt, minutes in g.timing:
                ...
                print(
                    ...
                    f"{FormatDatetime(start):^{w[2]}s}{indent}"
                    f"{FormatDatetime(finish):^{w[3]}s}"
                )
            print(
    ...
    def Test():
        ...
        if 1:  # FormatTime()
            Assert(FormatTime(0) == "12:00 am")
            Assert(FormatTime(1) == "12:01 am")
            Assert(FormatTime(1*60) == " 1:00 am")
            Assert(FormatTime(8*60) == " 8:00 am")
            Assert(FormatTime(12*60) == "12:00 pm")
            Assert(FormatTime(20*60) == " 8:00 pm")
            Assert(FormatTime(23*60) == "11:00 pm")
            Assert(FormatTime(23*60 + 59) == "11:59 pm")
    def PrintTiming():
        ...
        if 1:  # Tuesday and Thursday
            ...
            for ckt, minutes in g.timing:
                ...
                print(
                    ...
                    f"{sep}{FormatTime(T):^{w[2]}s}"
                    f"{sep}{FormatTime(T + minutes):^{w[3]}s}"
                )
                T += minutes
            print(
