...
def GetInchBoltData():
    ...
    for line in data.split("\n"):
        ...
        inch.append(f)
    # Print report
    t.print(f"{t.inch}Inch-size bolts")
    print(f'''                            Recommended torque in {t.ftlb}ft*lb{t.n}
...
Size  UNC   UNF   UNC   UNF   UNC   UNF   UNC   UNF   UNC   UNF   UNC   UNF''')
    ...
    print()
    print(dedent('''
    Ref:  https://www.boltdepot.com/fastener-information/Bolts/US-Recommended-Torque.aspx
    The grade 2, 5, and 8 values are for steel bolts slightly lubricated.

    '''))
    print()
    print(dedent('''
    My 3/4 Plomb breaker bar has an effective lever arm of about 20 inches.  Using a
    1-1/8 inch 1/2 inch drive socket on a 6" extension and a 3/4 to 1/2 adapter, I can
    just loosen the 3/4 UNF bolts on our load transferring hitch, meaning it's around
    170 ft*lb.
                                          Proof load kpsi     Yield, min, kpsi
    Grade 2:  no markings, low C steel          55                  57
    Grade 5:  3 lines, med. C steel HT          85                  92
    Grade 8:  6 lines, med. C steel HT         120                 130
    ''')
    )
def GetMetricBoltData():
    ...
    for line in data.split("\n"):
        ...
        metric.append([dia] + s)
    # Print report
    t.print(f"\n{t.metric}Metric bolts")
    print(f'''
                  Recommended torque in {t.ftlb}ft*lb{t.n}
                          Bolt Grade 
...
def Torques():
    ...
    print()
    print(dedent(f'''
    {t.inch}Wrench Torque Measurements{t.n} (using calibrated torque wrench on 3/8 bolt,
    so wrench/socket was 9/16 inches).  These measurements were made when I was 66 and
    are my personal limits for what I can apply with one hand horizontally at the bench.
    Much of the limiting factor was how uncomfortable the tool was in my hand.  The most
    comfortable tools were the two 3/8 inch drive ratchets marked with *.  The Force
    column is the calculated force I applied to the tool -- and note they are pretty
    consistent for the short versus long tools.'''))
    print(f'''
                                Torque,    ELA,     Force,
 No.         Wrench             {t.ftlb}ft-lbf{t.n}    inches     lbf    Model
 ---   ---------------------    ------    ------    ------  -----
  ...
  At age 75, I can lift comfortably with one hand at least 90 lbf measured (luggage
  gauge) on a breaker bar on a tire.  With a 2 foot breaker bar, this should break any
  typical car or RV lug nut loose.''')
