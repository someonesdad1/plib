'''
Prints combinations of gauge blocks that will sum to a desired size.  This
is a brute-force solution of the subset sum problem.
 
Note:  26 Dec 2014:  I checked my 'user' gauge block set's blocks that were
1 inch and under with a Starrett #216 micrometer that I got new in July of
1980.  Here are the deviations in 0.0001 inch units from the nominal size:
    ...
    0.950   -2
    1.000   -1
Note these measurements are more likely to be the micrometer's inaccuracy
rather than the gauge blocks' inaccuracies.  But they do give a measure of
confidence in the consistency of the gauge blocks.
'''
...
if 1:   # Global variables
    ...
    # want to add a new set.  Note the values are integers; this helps make
    # the summing of sizes fast and exact because there is no roundoff
    # error.
    gauge_block_sets = {
...
def Usage(d, status=1):
    ...
    Usage:  {name} [options] size1 [size2...]
      ...
      You can use python expressions for the sizes; the math module's
      symbols are in scope.  For example, a gauge block stack for a length
      of 25/32 of an inch could be gotten by the size "3/4 + 1/32",
      assuming the gauge block dimensions are in inches.
    Options:
      -a        Show all combinations that make up the desired size, not just
                the first combination found.
      -f pct    Tolerance percentage for -g/-G searches.  Defaults to {tol}%.
      -g dia    Display common gauge sizes for dia in inches
      -G dia    Same as -g, but all gauge sizes are shown
      -k num    Set the maximum subset size of blocks to use in the search.
      ...
      -T        For the user blocks, print out the dimensions up to 10 inches
                that cannot be gotten with {maxnum} blocks.
    '''))
