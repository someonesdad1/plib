...
def Usage(d, status=1):
    ...
    Usage:  {name} [options] [file1 [file2...]]
      For each file, find words with British spelling and print out the
      American form.  Include the file and line number.  Use a file of
      '-' for stdin.
    '''))
    exit(status)
def ParseCommandLine(d):
    ...
    return args
def Process(file):
    '''Print a list of British words in the file along with the line
    number.
    '''
    ...
    for ln, line in enumerate(lines):
        ...
        for word in words:
            ...
            if w in british_words:
                out[w].append(ln + 1)
    if out:
        print("stdin" if file == "-" else file, end=" ")
        ...
        for w in sorted(out):
            ...
            l = ' '.join(W)
            s = f"  {w} <-> {us}:  "
            if len(l) + len(s) <= width - 2:
                print(f"{s}{l}")
            else:
                print(s)
                for line in Columnize(W, indent=" "*4):
                    print(line)
