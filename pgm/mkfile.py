...
if 1:  # Utility
    ...
    def Usage(status=1):
        ...
        Usage: {sys.argv[0]} [options] size filename [n]
          ...
          1, 2, ... to the filename.  If filename has an extension, the added number will be before the
          extension.
     
          The default is to fill the file with random bytes using the secrets.token_bytes() method.
          The -r method is about 3 times slower than the other methods.
        Options:
...
if __name__ == "__main__":
    ...
    if n is not None:
        ...
        template = f"{p}{{}}"
        if p.suffix:
            template = f"{p.parent / p.stem}{{}}{p.suffix}"
        for i in range(1, n + 1):
            filename = template.format(i)
            MakeFile(size, filename)
    else:
        MakeFile(size, file)
