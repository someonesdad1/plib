...
def GetUnits(
    GetUnitData=DefaultUnitData,
    level=-1,
    show=False,
    randomize=False,
    angles_have_dim=False,
    check=False,
):
...
class Dim(object):
    ...
    def __init__(
        self, s="", ignore_case=True, allowed_symbols=tuple("M L T A K N C ↀ".split())
    ):
...
class U(object):
    ...
    def __init__(self, units, dimensions, prefixes={}, special={}, check=True):
    ...
    def _expand_units(self, expr, strict=False):
        ...
        for toknum, string, _, _, _ in tokenize.tokenize(t):
            ...
            if toknum == tokenize.NAME and string not in self._units:
                ...
                if (
                    string in self._prefixes
                    and string not in self._units
                    and not _strict
                ):
                    ...
                    if r:
                        ...
                        if (last_num == OP and last_token == ")") or (last_num == NAME):
                            ...
                            if not (last_num == OP and last_token == "("):
                                raise ValueError(
                                    "Negative signs or hyphens are not allowed"
                                )
...
def FormatUnit(unit, expr=False, flat=False, solidus=False, strict=False, sep="·"):
