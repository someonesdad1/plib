...
if 1:   # Header
    ...
    if 1:   # Global variables
        ...
        ReadabilityTuple = namedtuple("Readability", '''
            FK_ease
            FK_grade
            DaleChall
            ''')
...
if 1:   # Basic routines
    def GuessSyllables(word, letters_only=False):
        ...
        #
        syl = 0
        subtract_syl = ['cial', 'tia', 'cius', 'cious',
                        'giu',              # belgium!
                        'ion', 'iou', 'sia$',
    ...
    def GetTextInfo(text):
        ...
        s = [characters, words, complex_words, one_syllable_words,
             syllables, sentences, wordlist]
        for i in s[:-1]:
            Assert(i >= 0)
        return TextInfo(*s)
