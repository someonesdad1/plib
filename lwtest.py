...
Lightweight testrunner framework
    ...
    function objects.  Call run(verbose=True) to see which functions will
    ...
    Possible enhancements:
        ...
        * If an argument passed on the command line is a directory, search
        ...
        run them.
            ...
            suitable test functions and run them, even if there's no run()
            call in the script.  Options to provide run()'s features:  -h
            to halt at first failure, -r for regexp to identify a test
            function, -R for regexp's options, -v for verbose
        * assert_equal:  add a dict keyword so that when dictionaries are
        compared, both keys and values are compared.  Consider adding
        sets to the function too.
...
'''
    ...
    # The raises context() manager functionality was copied from pytest's
