...
from 0 to n - 1, where n is the size of the bitfield.
  - There are two implementations:  sbitfield uses a list of string characters and
    bitfield is derived from an integer.  The code for bitfield is simpler, but it's
    slower for larger numbers of bits.
  ...
  - On 8 Jun 2021 I reimplemented the class bitfield as derived from an integer, as it
    makes the implementation simpler.  However, it's slower for larger bitfields:
  ...
  - 13 Aug 2025 timings:
      - sbitfield timing:
        - Size 1.0e+06 time = 0.00 sec
        - Size 1.0e+07 time = 0.00 sec
        - Size 1.0e+08 time = 0.01 sec
        - Size 1.0e+09 time = 0.05 sec
        - Size 1.0e+10 time = 0.50 sec
        - Size 1.0e+11 time = 5.10 sec
      - bitfield timing:
        - Size 1.0e+06 time = 0.00 sec
        - Size 1.0e+07 time = 0.02 sec
        - Size 1.0e+08 time = 0.32 sec
        - Size 1.0e+09 time = 4.24 sec

      - Comments
        - It's interesting that the sbitfield implementation using a list of strings is
          85 times faster than bitfield
        - The most obvious implementation of a bitfield today is a bytearray structure
