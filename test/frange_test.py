...
def test_mpmath():
    try:
        ...
        print("{}:  Warning:  mpmath not tested".format(__file__), file=sys.stderr)
    else:
        ...
        mp.dps = 16
        got = list(frange("9.6001", "9.601", "0.0001", return_type=mpf, impl=mpf))
...
def test_numpy():
    try:
        ...
        print("{}:  Warning:  numpy not tested".format(__file__), file=sys.stderr)
    else:
