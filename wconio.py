...
DP's notes:
    - Downloaded Sat 23 Dec 2023
        - From https://github.com/Solomoriah/WConio2/WConio2.py The
          original file's MD5 hash was df55e48442e66024a34acde05fe29133 and
          it was 15389 bytes.
    - This is a reimplementation of the original python wconio module for
      Windows.
    - I have modified the style but not the functionality (except for
      adding a quick 'if __name__ == "__main__"' section for a test),
      primarily to allow easier viewing in a folding editor.
...
if 1:  # Internal functions
    ...
    def _getscreeninfo():
        ...
        hConOut = _getconout()
        if BOOL(kernel32.GetConsoleScreenBufferInfo(hConOut, ctypes.byref(csbi))):
            _releaseconout(hConOut)
            return csbi
        _releaseconout(hConOut)
