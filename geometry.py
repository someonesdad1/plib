...
A collection of plane geometry formulas.  This module will not work under
python 2.
...
def Quadrilateral(p1, p2, p3, p4):
    ...
    A = abs((((y[0]*x[1]) + (y[0]*x[3]*(-1)) + (y[1]*x[0]*(-1)) +
            (y[1]*x[2]) + (y[2]*x[1]*(-1)) + (y[2]*x[3]) + (y[3]*x[0])
            + (y[3]*x[2]*(-1)))*1/2))
...
def QuadrilateralPerimeter(p1, p2, p3, p4):
    ...
    x = [p1[0], p2[0], p3[0], p4[0]]
    y = [p1[1], p2[1], p3[1], p4[1]]
    return (((((x[0] + (x[1]*(-1))))**(2) + ((y[0] +
            (y[1]*(-1))))**(2)))**(1/2) + (((((x[0]*(-1)) + x[3]))**(2) +
            (((y[0]*(-1)) + y[3]))**(2)))**(1/2) + ((((x[1] +
            (x[2]*(-1))))**(2) + ((y[1] + (y[2]*(-1))))**(2)))**(1/2) +
            ((((x[2] + (x[3]*(-1))))**(2) + ((y[2] +
            (y[3]*(-1))))**(2)))**(1/2))
...
def QuadrilateralAngles(p1, p2, p3, p4):
    ...
    try:
        a = atan((((((y[0]*(-1)) + y[3])*((x[2]*(-1)) + x[3])*(-1)) +
                 ((y[2] + (y[3]*(-1)))*(x[0] + (x[3]*(-1)))))*(((((x[2]*(-1)) +
                 x[3])*(x[0] + (x[3]*(-1)))) + ((y[2] + (y[3]*(-1)))*((y[0] *
                 (-1)) + y[3]))))**((-1))))
    except ZeroDivisionError:
        a = pi/2
    try:
        b = (pi + (atan(((((y[0] + (y[1]*(-1)))*((x[1]*(-1)) +
             x[2])*(-1)) + ((y[1] + (y[2]*(-1)))*((x[0]*(-1)) +
             x[1])))*(((((x[1]*(-1)) + x[2])*((x[0]*(-1)) + x[1])) + ((y[1] +
             (y[2]*(-1)))*(y[0] + (y[1]*(-1))))))**((-1))))*(-1)))
    except ZeroDivisionError:
        b = pi/2
    try:
        c = (pi + (atan(((((y[0] + (y[1]*(-1)))*(x[0] + (x[3]*(-1)))) +
             (((y[0]*(-1)) + y[3])*((x[0]*(-1)) + x[1])*(-1)))*(((((x[0] *
             (-1)) + x[1])*(x[0] + (x[3]*(-1)))) + (((y[0]*(-1)) + y[3]) *
             (y[0] + (y[1]*(-1))))))**((-1))))*(-1)))
    except ZeroDivisionError:
        c = pi/2
    d = 2*pi - (a + b + c)
...
QuadrilateralDiagonals((0, 0), (1, 0), (0, 1), (1, 1))
