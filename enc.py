...
if 1:  # Core functionality
    ...
    def Is8859(file):
        ...
        r = subprocess.run(["/usr/bin/file", str(file)], capture_output=True)
        if r.returncode:
            Error(f"Running /usr/bin/file on {str(file)} failed")
        return b"ISO-8859" in r.stdout
    def Convert8859(file):
        ...
        if not Is8859(file):
            ...
            return 1
        mybytes = file.open("rb").read()
        # Create a UTF8 string copy
        s = str(mybytes, encoding=e)
        # Write to a file
        newfile = Path(str(file) + ".utf8")
        ...
        try:
            ...
            t.print(f"{t.redl}Unable to remove {str(file)!r}", file=sys.stderr)
            newfile.unlink()
            return 3
        ...
        try:
            ...
            t.print(f"{t.redl}Unable to rename {str(newfile)!r} to {str(file)!r}", file=sys.stderr)
            return 4
        return 0
