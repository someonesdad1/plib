...
def RoundedRectangle(file):
    ...
    y = 10.5
    move(0.5, y)
    text("Rounded rectangles:  the number in the box is the corner diameter")
    ...
    x_box_sep = 0.5
    y_box_sep = 0.5
    column_width = (page_width - left_margin - right_margin) / float(num_columns)
    row_height = (page_height - bottom_margin - top_margin) / float(num_rows)
    ...
    for row in range(num_rows):
        for column in range(num_columns):
            ...
            corner_diameter = max_corner_diameter * boxnum / (num_boxes - 1)
